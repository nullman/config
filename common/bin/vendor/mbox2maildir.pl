...
# Public domain.
#
# Juri Haberland <juri@koschikode.com> 1 October 2002
# Robin Whittle  12 March 2002
# Jeff Williams   8 November 2002
# Geoffroy DESVERNAY <dgeo@dgeos.net>  13 January 2003
#
...
#
# Original version tested on RedHat 7.1 Perl 5.6.0
# Version 2.01 tested on     RedHat 7.2 Perl 5.6.0
# Version 2.03-beta with '-R' tested on FreeBSD 4.7-STABLE (Perl 5.005), Debian 3.0(woody) (Perl 5.6.1)
# Version 3.00 tested on RedHat 7.2 & 7.3 (Perl 5.6.1)
# Version 3.10 tested on Gentoo Linux 1.4 (Perl 5.8.0)
#
#
# Jeff Williams:
#
...
# Changes 8/11/2002:
#
# - put the mbox to maildir conversion into it's own function
# - made it possible to specify a source mbox file, rather
# 	than just the directory
# - destination can now be an absolute path instead of relative
# 	to the users home directory
# - the users mail directory for -m is found from $ENV{MAIL}
# 	rather than assumed to be in /var/spool/mail/$user
# - mbox files with .'s in their name have them converted to -'s
# - cleaned up a lot of the code and removed some unnecessary
# 	variables. Added the use strict pragma.
#
# --------------------------------------------------------------------
#
# Geoffroy DESVERNAY <dgeo@dgeos.net> 13 January 2003
# 
# Ok, let's put changelog here :)
# Changes 01/13/2003
# 
# - added the '-R' switch, to permit 'one-pass conversion'
# - added a isamailboxfile() function to skip some 'address-book' or other
#   files I found in users's 'Mail' directory
# - added a convertit() function to permit recursion
# - added a maildirmake() function to create empty folders (used the same code
#   that used to create it)
# - added a '/bin/date' call in convert() to convert the date in case of *BSD 
#   system : it's now  usable for a FreeBSD 4.7 system
# - added a $touchtimeswitch variable (*BSD touch need '-t' instead of '-d')
# - I tried not to change the default behaviour... hope it worked !
#
# --------------------------------------------------------------------
#
# Juri Haberland <juri@koschikode.com> 2003/01/24
#
# - fixed a bug where dots (.) in foldernames weren't converted to '_'
# - fixed a bug where an empty file was created if we have a folder
#   that has just a dummy message in it
# - incoorperated changes by Kjetil Torgrim Homme:
#   handle "From " correctly
# - incoorperated changes by Ludovic Robinot:
#   also use X-Mozilla-Status flags (local Mozilla mailboxes)
# - minor cleanups
#
# --------------------------------------------------------------------
# Juri Haberland <juri@koschikode.com> 2003/02/01
#
# - bugfix: $unamecmd was set to /usr/bin/touch; grmbl cut-n-paste bug
#   thanks to Nick Triantos
#
# --------------------------------------------------------------------
#
# Juri Haberland <juri@koschikode.com> 2003/02/14
#
# - bugfix: if dummy message is present the last converted mail wasn't
#   touched with the parsed date
# - patch by Wayne Davison:
#   use Date::Parse from the TimeDate package to parse the From date
#   and use the perl function utime() instead of touch; minor code
#   cleanups
#
# --------------------------------------------------------------------
#
...
#
#   ------- Mozilla mailbox message status interpretation ------
#
#   19-dec-2002 - Ludovic Robinot (robinot@geovariances.fr)
#
#   With this patch, I tried to convert Mozilla X-Mozilla-Status: NNNN flags to
#   imap status.
#
#   You will find information on flags values here:
#   http://lxr.mozilla.org/seamonkey/source/mailnews/base/public/nsMsgMessageFlags.h
#
#   0x0001 -> R -> S     Read = Seen
#   0x0002 -> A -> R     Replied
#   0x0008 -> D -> T     Tagged for deletion
#
#   ------- Mozilla mailbox message status interpretation ------
#
...
sub usage() {
    print "Usage:\n";
    print "       mb2md -h\n";
    print "       mb2md -m [-d destdir]\n";
    print "       mb2md -s sourcefile [-d destdir]\n";
    die   "       mb2md -s sourcedir [-R|-f somefolder] [-d destdir] [-r strip_extension]\n";
}
		    # get options
my %opts;
getopts('d:f:hms:r:R', \%opts) || usage();
...
sub isamailboxfile {
	my ($fichier) = @_;
	return 1 if(-z $fichier);
	open(MBXFILE, "$fichier") or die "Could not open $fichier ! \n";
	while(<MBXFILE>) {
		if ((/^From/)||(/^\s*$/)) {
			close(MBXFILE);
			return 1;
			...
			utime $t, $t, $messagefn;
                ...
                if ($fromline =~ m/@/)
                {
                    ...
                    $fromline =~ s/@//;
                            ...
                            #
                            # If there wasn't an "@" in the line, then we have
                            # just a raw word, such as an email account name with
                            # no "@domain.com" after it.
                            #
                            # In either case, we want to delete the word which
                            # follows and the space which follows it.  Then,
                            # all that should be left is the date-time.

                $fromline =~ s/(\S)+ *//;

			    # remove a possible trailing timezone setting,
			    # which will look like "+0100" or "-0800"
               ...
               )
            {
                ...
                if ($_ eq "\n")
                {
                    ...
                    $inheaders = 0;
                            ...
                            # Note that Philip Mal's perfect_maildir put the
                            # converted messages in /cur/ rather than /new/
                            # This makes some sense because they are not
                            # really freshly recieved.
                            #
                            # A simple version of this would be:
                            #
                            #   $messagefn = "new/$unique.$messagecount.mbox:2,";
                            #
